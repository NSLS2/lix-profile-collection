...
class Columns():
    ...
    '''
    column_parameters = {"column_type": {"Superdex" : {"high_pressure_limit" : 32, "max_flow_rate" : 0.35}, 
                                   "dSEC2":{"high_pressure_limit" : 120, "max_flow_rate" : 0.35}, 
                                   "Superose_6": {"high_pressure_limit" : 32, "max_flow_rate" : 0.35}, }}
    def __init__(self, 
                column_type = None,
                pressure = None,
                flow_rate = None,
                location = None):
        self.column_type = column_type if column_type is not None else self.column_type
        self.pressure_sv = pressure if pressure is not None else 1
        self.high_pressure_limit
...
class SEC_SAXSCollection(Columns):
    ...
    def __init__(self,
                exp_type = None,
                buff_pos = None,
                sec_col_pos = {'col_pos1': {"status" : True, "column_type" : "Superdex"},
                              'col_pos2': {"status" : False, "column_type" : "dSEC2"}}
                ):
        super().__init__()
        self.column_locations = sec_col_pos
    ...
    def update_column_positions(self, key, new_status):
        if key in self.column_locations:
            self.column_locations[key]["status"] = new_status
            return self.column_locations
        else:
            print(f"{key} not a valid position for a column.  Keys are col_pos1 or col_pos2")
    def update_column_type(self, key, column_type):
        if key in self.column_locations:
            self.column_locations[key]["column_type"] = column_type
        else:
            print("check key or column_type, no change")
    ...
    """
        strFields=["Vial", "Sample Name", "Sample Type", "Acq. method", "Proc. method", "Data file", "Buffer", "Valve Position", "Exp_Type"]
    ...
    def safe_to_change(self, curr_pressure=None, curr_high_press_limit=None, pressure_sv=None,col_max_pressure=None, 
                       ...
                       regen_high_press_limit=None, regen_col_max_pressure=None):
        """
