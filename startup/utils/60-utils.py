...
def make_video_from_tiled(uid, det_name="camES2", fps=3, figsize=(4,4), **kwargs):
    """ assume the data is under f"primary/data/{det_name}"
    ...
    sn = c[uid].start['sample_name']
    images0 = c[uid][f'primary/data/{det_name}_image']
    images = np.reshape(images0, [-1, *images0.shape[-3:]])
    nfr = len(images)
    ...
    vidwriter = cv2.VideoWriter(f"{sn}-video.mp4", cv2.VideoWriter_fourcc(*"mp4v"), fps, (width,height))
    for img in frames:
